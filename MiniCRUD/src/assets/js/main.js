...
arrayUsuarios.forEach((usuario, posicionEnLista) => {
        ...
        nodoFila.innerHTML = `
            ...
            <td>${convertirATextoSeguro(usuario?.rol ?? '')}</td> <td>
                <button
                    type="button"
...
console.log('Datos de la API:', cuerpoJson.data);
...
if (!datosUsuarioNuevo.nombre || !datosUsuarioNuevo.email || datosUsuarioNuevo.rol === '' || !datosUsuarioNuevo.password) {
mostrarMensajeDeEstado('error', 'Los campos Nombre, Email, Rol y Contraseña son obligatorios.');
return;
}
try {
